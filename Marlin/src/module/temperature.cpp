...
void Temperature::min_temp_error(const heater_id_t heater_id) {
  ...
  float Temperature::get_pid_output_hotend(const uint8_t E_NAME) {
    const uint8_t ee = HOTEND_INDEX;
        ...
        static bool heat[HOTENDS] = { true };
        static int custom_pinheat[] = CUSTOM_PINHEAT;
        static bool custom_heatstate[] = CUSTOM_PINSTATE;
        static int custom_range_heate[] = CUSTOM_RANGE_HEATE;
        static int custom_range_coole[] = CUSTOM_RANGE_COOLE; 
        ...
        int16_t cust_temp_range[HOTENDS];

        #ifdef CUSTOM_TARGETE_DIVIDE
          static int cust_targete_divide[HOTENDS] = CUSTOM_TARGETE_DIVIDE;
          static int cust_targete_range[HOTENDS] = CUSTOM_TARGETE_RANGE;
          if (temp_hotend[ee].target >= cust_targete_divide[ee]) cust_temp_range[ee] = cust_targete_range[ee];
          else cust_temp_range[ee] = custom_range_heate[ee];
        #else
          cust_temp_range[ee] = custom_range_heate[ee];
        #endif

        if (custom_pinheat[ee] == 0) {
          heat[ee] = true;
        }

        else if ((custom_pinheat[ee] != 0) && (digitalRead(custom_pinheat[ee]) == custom_heatstate[ee])) {
          heat[ee] = true;
        }
        ...
        //heating
        else if (heat[ee] && ((temp_hotend[ee].target + custom_range_heate[ee]) < temp_hotend[ee].celsius)) {
          pid_output = 0;
          pid_reset[ee] = false;
        }

        //cooling
        else if (!heat[ee] && ((temp_hotend[ee].target + custom_range_coole[ee]) > temp_hotend[ee].celsius)) {
          pid_output = 0;
          pid_reset[ee] = false;
        }

        else if (heat[ee] && pid_error > cust_temp_range[ee]) {
          pid_output = BANG_MAX;
          pid_reset[ee] = true;
        }

        else if (!heat[ee] && pid_error < custom_range_coole[ee]) {
          pid_output = BANG_MAX;
          pid_reset[ee] = true;
        }
         ...
         if (temp_hotend[ee].target > temp_chamber.celsius)
          {
  ...
  float Temperature::get_pid_output_bed() {
      ...
      static bool pid_reset = false;
      float pid_output = 0;
      const float pid_error = temp_bed.target - temp_bed.celsius;
      int16_t cust_temp_range;

      #ifdef CUSTOM_TARGETB_DIVIDE
        if (temp_bed.target >= CUSTOM_TARGETB_DIVIDE) cust_temp_range = CUSTOM_TARGETB_RANGE;
        else cust_temp_range = CUSTOM_RANGE_HEATB;

      #else
        cust_temp_range = CUSTOM_RANGE_HEATB;
      #endif

      #ifdef CUSTOM_HEATBED
        if (digitalRead(CUSTOM_HEATBED) == CUSTOM_BED_STATE) {
          heat = true;
        }

        else if (digitalRead(CUSTOM_HEATBED) != CUSTOM_BED_STATE) {
          heat = false;
        }
      ...
      //heating
      else if (heat && ((temp_bed.target + CUSTOM_RANGE_HEATB) < temp_bed.celsius)) {
        pid_output = 0;
        pid_reset = false;
      }

      //cooling
      else if (!heat && ((temp_bed.target + CUSTOM_RANGE_COOLB) > temp_bed.celsius)) {
        pid_output = 0;
        pid_reset = false;
      }
      
      else if (heat && pid_error > cust_temp_range) 
      {
        pid_output = MAX_BED_POWER;
        pid_reset = true;
      }

      else if (!heat && pid_error < CUSTOM_RANGE_COOLB) 
      {
        pid_output = MAX_BED_POWER;
        pid_reset = true;
      }
        ...
        else if (temp_bed.target > temp_chamber.celsius)
          {
          const float max_power_over_i_gain = float(PID_MAX) / temp_bed.pid.Ki - float(MIN_POWER);
